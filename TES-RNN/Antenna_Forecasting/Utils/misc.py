...
def categorical_sample(probs, use_cuda=False, use_log=True):
    if use_log:
        int_acs = torch.multinomial(torch.tensor(np.log(probs.detach().numpy() + 1)), 1)
    else:
...
def sep_clip_grad_norm(parameters, max_norm, norm_type=2):
    """
    Clips gradient norms calculated on a per-parameter basis, rather than over the whole list of parameters as in torch.nn.utils.clip_grad_norm.
